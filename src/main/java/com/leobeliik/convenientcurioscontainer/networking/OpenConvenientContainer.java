        ...
        ctx.get().enqueueWork(() -> {
            ...
            if (player != null) {
                ...
                if (mainItem.getItem() instanceof ConvenientItem) {
                    mainItem.getOrCreateTag().putInt("CustomModelData", 1);
                    NetworkHooks.openScreen(player, ConvenientMenuProvider.MenuProvider(mainItem));
                } else if (offItem.getItem() instanceof ConvenientItem stack) {
                    offItem.getOrCreateTag().putInt("CustomModelData", 1);
                    NetworkHooks.openScreen(player, ConvenientMenuProvider.MenuProvider(offItem));
                } else {
                    inv.stream().filter(itemStack -> itemStack.getItem() instanceof ConvenientItem stack).findFirst()
                            .ifPresent(itemStack -> {
                                itemStack.getOrCreateTag().putInt("CustomModelData", 1);
                                NetworkHooks.openScreen(player, ConvenientMenuProvider.MenuProvider(itemStack));
                            });
                }
            }
        });
